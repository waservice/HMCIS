...
namespace WA.Standard.IF.WebService.v2.Common
{
    ...
    public class Message : WA.Standard.IF.WebService.v2.Base.BaseWebService
    {
        ...
        public MessageSendResponse MessageSend(MessageSendRequest request)
        {
            ...
            try
            {
                WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA MessageSendRequest XML", request);
                ...
                if (response.Errors == null)
                {
                    ...
                    WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA MessageSendResponse XML", response);
                }
            }
            catch (Exception ex)
            {
                response.Errors = GetErrorDataListFromException(ex);
                WA.Standard.IF.Logger.Log.Log.SaveErrorLog(null, "MessageSend", request, ex.Message, ex);
            }
