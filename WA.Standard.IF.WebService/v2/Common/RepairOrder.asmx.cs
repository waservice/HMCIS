...
namespace WA.Standard.IF.WebService.v2.Common
{
    ...
    public class RepairOrder : WA.Standard.IF.WebService.v2.Base.BaseWebService
    {
        ...
        public RepairOrderChangeResponse RepairOrderChange(RepairOrderChangeRequest request)
        {
            ...
            try
            {
                WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA RepairOrderChangeRequest XML", request);
                ...
                if (response.Errors == null)
                {
                    ...
                    WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA RepairOrderChangeResponse XML", response);
                }
            }
            catch (Exception ex)
            {
                response.Errors = GetErrorDataListFromException(ex);
                WA.Standard.IF.Logger.Log.Log.SaveErrorLog(null, "RepairOrderChange", request, ex.Message, ex);
            }
