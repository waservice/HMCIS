...
namespace WA.Standard.IF.WebService.v2.HMCIS
{
    ...
    public class Print : WA.Standard.IF.WebService.v2.Base.BaseWebService
    {
        ...
        public PrintResponse PrintRequest(PrintRequest request)
        {
            ...
            try
            {
                WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA PrintRequest XML", request);
                ...
                if (response.Errors == null)
                {
                    ...
                    WA.Standard.IF.Logger.Log.Log.SaveXMLLog(0, request.TransactionHeader.DealerID, "WA PrintResponse XML", response);
                }
            }
            catch (Exception ex)
            {
                response.Errors = GetErrorDataListFromException(ex);
                WA.Standard.IF.Logger.Log.Log.SaveErrorLog(null, "PrintRequest", request, ex.Message, ex);
            }
